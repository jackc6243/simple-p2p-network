...
// creates a connection to the given ip and port, returns socket fd
int create_connection(char* ip, int port) {
    ...
    struct btide_packet* packet = create_packet(PKT_MSG_ACP);
    ssize_t nwritten = send(sock, packet, sizeof(packet), 0);
    if (nwritten <= 0) {
    ...
    // read message
    ssize_t nread = read(sock, packet, sizeof(packet));
    if (nread <= 0) {
        fprintf(stderr, "Server disconnected: socket fd %d\n", sock);
        close(sock);
        return -1; // failed
    } else if (packet->msg_code != PKT_MSG_ACK) {
